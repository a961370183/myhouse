...
public class HouseRouter {
    ...
    @RequestMapping("/houseDetail/{houseno}")
    public String userHouseDetail(Model mv, @PathVariable int houseno ){
        ...
        if(houseDetailImpl.getUserHouse(houseno)!=null){
            mv.addAttribute("house",houseDetailImpl.getUserHouse(houseno));
        }else if(houseDetailImpl.getAgentHouse(houseno)!=null){
    ...
    @RequestMapping("/houseQA")
    public String houseQA(Model mv,int questionNo){
        User user = userService.userInfo();
        List answers = answerServiceImpl.getAnswerOfQuestion(questionNo);
        Question question = questionService.getQuestion(questionNo);

        mv.addAttribute("userInfo",user);
        mv.addAttribute("question",question);
        mv.addAttribute("answers",answers);
        return "house/houseQA";
    }

    @RequestMapping("/houseQA/addAnswer")
    @ResponseBody
    public List addAnswer(String questionId,String content){
        User user = userService.userInfo();
        List list = null;
        if(user!=null){
            try {
                int qId = Integer.parseInt(questionId);
                answerServiceImpl.addAnswer(qId, content, user.getUserId());
                list = answerServiceImpl.getAnswerOfQuestion(qId);
            }catch (Exception e){
    ...
    public String houseUpload(Model mv, @PathVariable int houseNo, HttpServletResponse response) throws Exception{
        ...
        if(role==1){
            ...
            if(userHouseDetail!=null){
                if(userHouseDetail.getUserId() != user.getUserId()){
