...
public interface ContractingMapper {
    ...
    @Select("select * from contracting where userId=#{userId} and workNo=#{workNo}")
    ...
    @Insert("insert contracting (userid,workno,created) values (#{id},#{workNo},#{created})")
    void setContracting(int id, int workNo, String created);

    @Select("select * from contracting where userId=#{userId}")
