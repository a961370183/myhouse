...
public class AgentController {
    ...
    @RequestMapping("sign")
    @ResponseBody
    public String sign(HttpServletRequest request, @RequestParam(value = "userId", defaultValue = "")int id, @RequestParam(value = "id", defaultValue = "")int workNo, @RequestParam(value = "name", defaultValue = "") String name,@RequestParam("file") MultipartFile[] files) throws FileNotFoundException {
        String finalUrl = "";
        System.out.println("长度为："+ files.length);
        for (MultipartFile file : files) {
            finalUrl += imgTools.saveFile(file, "images/agent/contract") + ";";
        }
        System.out.println("url是"+finalUrl);
        if (finalUrl.length() > 0)
            finalUrl = finalUrl.substring(0, finalUrl.length() - 1);
        return agentService.sign(id,workNo,name,finalUrl);
    }
